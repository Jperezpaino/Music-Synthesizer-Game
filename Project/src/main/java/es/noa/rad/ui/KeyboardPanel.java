...
public final class KeyboardPanel extends JPanel {
    ...
    this.addMouseListener(new MouseAdapter() {
      ...
      public void mouseReleased(final MouseEvent e) {
        if (KeyboardPanel.this.pressedNote != null) {
          ...
          KeyboardPanel.this.soundEngine.stopNote(released);
  ...
  /**
   * Pulsa una nota programáticamente (usado por el mapeo de teclado PC).
   *
   * @param note Nota a pulsar
   */
  public void pressNote(final Note note) {
    if (note == null || note.equals(this.pressedNote)) {
      return;
    }
    this.pressedNote = note;
    this.soundEngine.startNote(note);
    if (this.noteListener != null) {
      this.noteListener.onNotePressed(note);
    }
    repaint();
  }

  /**
   * Suelta una nota programáticamente (usado por el mapeo de teclado PC).
   *
   * @param note Nota a soltar
   */
  public void releaseNote(final Note note) {
    if (note != null && note.equals(this.pressedNote)) {
      this.soundEngine.stopNote(note);
      this.pressedNote = null;
      if (this.noteListener != null) {
        this.noteListener.onNoteReleased(note);
      }
      repaint();
    }
  }
