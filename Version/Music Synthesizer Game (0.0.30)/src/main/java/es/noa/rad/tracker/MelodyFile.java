...
public final class MelodyFile {
  ...
  private static final String KEY_STEPS      = "_STEPS";
  ...
  private static boolean writeToFile(final File file,
    ...
    try (final BufferedWriter bw = new BufferedWriter(new FileWriter(file))) {
      ...
      for (int i = 0; i < trackers.length; i++) {
        ...
        final boolean[][] steps = tp.getSteps();
    ...
    try (final BufferedReader br = new BufferedReader(new FileReader(file))) {
      ...
      while ((line = br.readLine()) != null) {
        ...
        if (line.startsWith(KEY_BPM + "=")) {
          ...
          // Formato multi-canal: CHANNEL_N_PROGRAM / _SCALE / _STEPS
          for (int i = 0; i < n; i++) {
            final String pfx = KEY_CH_PFX + i;
      ...
      // Aplicar retrocompatibilidad: si no hay datos de canal 0, usar legacy
      if (legacyScale != null) scales[0] = legacyScale;
      if (legacySteps != null) stepLists[0] = legacySteps;

      // Aplicar a todos los canales
      for (int i = 0; i < n; i++) {
        trackers[i].clearAll();
        trackers[i].setScale(scales[i]);
        for (final int[] rc : stepLists[i]) {
          trackers[i].setStep(rc[0], rc[1], true);
        }
