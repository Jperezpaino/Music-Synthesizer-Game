...
public final class SequencerEngine {
  ...
  private void tick() {
    ...
    for (final TrackerPanel tracker : this.trackers) {
      ...
      tracker.setPlayColumn(col);

      // Reproducir todas las notas activas en esta columna usando el canal MIDI del panel
      final int chIdx = tracker.getChannelIndex();
      for (int row = 0; row < TrackerPanel.ROWS; row++) {
        if (tracker.isRowActive(row) && tracker.isStepActive(row, col)) {
