...
public final class TrackerApplication {
  ...
  private static void launchWindow() {
    // Instalar el Look & Feel nativo del sistema operativo (Windows en este caso)
    // para que menus, dialogos, combos y scrollbars tengan apariencia Windows nativa.
    try {
      UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
    } catch (final Exception ignored) {
      // Si falla (p.ej. en otro SO) se usa el L&F por defecto de Swing sin problema
    }

    // Aplicar las personalizaciones de color encima del L&F del sistema
    applyLookAndFeel(true);

    // Pre-cargar el sintetizador MIDI para evitar retardo en la primera nota
    NotePlayer.warmUp();
    ...
    // -- Menu bar
    frame.setJMenuBar(buildMenuBar(tracker, frame, sequencer));

    // -- Toolbar
    final JPanel toolbar = buildToolbar(tracker, frame, sequencer);
    frame.add(toolbar, BorderLayout.NORTH);

    // Al editar cualquier celda, detener la reproduccion automaticamente
  ...
  private static JMenuBar buildMenuBar(final TrackerPanel tracker,
                                       final JFrame frame,
                                       final SequencerEngine sequencer) {
    ...
    openItem.addActionListener(e -> {
      ...
      final File loaded = MelodyFile.load(frame, tracker, sequencer,
    ...
    saveItem.addActionListener(e -> {
      if (currentFile != null) {
        MelodyFile.saveToFile(currentFile, tracker, sequencer, frame);
        frame.setTitle(DEFAULT_TITLE + " — " + currentFile.getName());
      } else {
        // Sin fichero abierto: comportarse como Guardar como
        final File saved = MelodyFile.saveAs(frame, tracker, sequencer);
        if (saved != null) {
          currentFile = saved;
    ...
    exportItem.addActionListener(e ->
        WavExporter.export(frame, tracker, sequencer));
    fileMenu.add(exportItem);

    fileMenu.addSeparator();

    // -- Cerrar (Ctrl+W) — limpia todo y vuelve al estado inicial
    ...
    closeItem.addActionListener(e -> {
      stopSequencer(frame, sequencer);
      tracker.clearAll();
      tracker.setScale(TrackerScale.EASY_PLUS);
  ...
  private static JPanel buildStatusBar(final TrackerPanel tracker, final JFrame frame) {
    ...
    themeBtn.addActionListener(e -> {
      ...
      // Reinstalar el L&F nativo para que los componentes estandar no retrocedan
      // al L&F por defecto de Swing al llamar updateComponentTreeUI
      try {
        UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
      } catch (final Exception ignored) { /* usar L&F actual si falla */ }
