...
public final class SoundEngine {
  ...
  /** Preset de instrumento activo */
  private InstrumentPreset preset;

  /** Forma de onda actual (puede sobreescribirse independientemente del preset) */
  private Waveform waveform;

  /** Volumen actual (0.0 - 1.0) */
  private double volume;

  // --- Parámetros ADSR del preset activo ---
  ...
  /** Cadena de efectos de audio aplicada a cada sample generado */
  private final EffectChain effectChain;

  /** Efecto de delay (acceso directo para la UI) */
  private final DelayEffect delayEffect;

  /** Efecto de reverberación (acceso directo para la UI) */
  private final ReverbEffect reverbEffect;

  /** Efecto de distorsión (acceso directo para la UI) */
  private final DistortionEffect distortionEffect;

  /**
   * Crea un motor de audio con el preset Piano por defecto.
   */
  public SoundEngine() {
    ...
    // Inicializar efectos (desactivados por defecto)
  ...
  /**
   * Bucle principal de generación y envío de audio con envolvente ADSR completa.
   *
  ...
  private void playLoop(final double frequency) {
    try {
      ...
      // Snapshots locales del ADSR para que no cambien a mitad de nota
      final int    attack  = this.attackSamples;
      final int    decay   = this.decaySamples;
      final double sustain = this.sustainLevel;

      while (this.playing) {
        for (int i = 0; i < BUFFER_SIZE; i += 2) {
          final double sample   = this.generateSample(frequency, sampleIndex);
          final double envelope = this.computeADSR(sampleIndex, attack, decay, sustain);
          final double dry      = sample * envelope * this.volume;
          final double finalSample = this.effectChain.process(dry);

          // Buffer circular del osciloscopio
  ...
  /**
   * Calcula el valor de la envolvente ADSR para un sample dado.
   *
   * @param idx     Índice del sample desde el inicio de la nota
   * @param attack  Samples de ataque
   * @param decay   Samples de decaimiento
   * @param sustain Nivel de sustain (0.0–1.0)
   * @return Valor de envolvente entre 0.0 y 1.0
   */
  private static double computeADSR(final long idx,
                                    final int attack,
                                    final int decay,
                                    final double sustain) {
    if (attack > 0 && idx < attack) {
      // Ataque: rampa lineal 0 → 1
      return (double) idx / attack;
    }
    final long afterAttack = idx - attack;
    if (decay > 0 && afterAttack < decay) {
      // Decaimiento: rampa lineal 1 → sustain
      return 1.0 - (1.0 - sustain) * ((double) afterAttack / decay);
    }
  ...
  /**
   * Aplica un decaimiento (release) suave al soltar la tecla para evitar clicks.
   *
   * @param frequency   Frecuencia en Hz
   * @param sampleIndex Índice de sample en el que se dejó de pulsar
   */
  private void applyRelease(final double frequency, final long sampleIndex) {
    if (this.line == null) {
      return;
    }
    final int release = Math.max(200, this.releaseSamples);
    final byte[] buffer = new byte[release * 2];
    for (int i = 0; i < release; i++) {
      final double sample     = this.generateSample(frequency, sampleIndex + i);
      final double fadeOut    = 1.0 - ((double) i / release);
      final double finalSample = sample * fadeOut * this.sustainLevel * this.volume;
      final short sampleShort = (short) (finalSample * Short.MAX_VALUE);
  ...
  /**
   * Cambia la forma de onda del sintetizador (sobrescribe la del preset).
   *
  ...
  /**
   * Aplica un preset de instrumento: waveform + parámetros ADSR.
   *
  ...
  /** Aplica internamente el preset (también usado desde el constructor). */
  ...
  /** @return Efecto de delay (para configuración desde la UI) */
  public DelayEffect getDelayEffect() {
    return this.delayEffect;
  }

  /** @return Efecto de reverberación (para configuración desde la UI) */
  public ReverbEffect getReverbEffect() {
    return this.reverbEffect;
  }

  /** @return Efecto de distorsión (para configuración desde la UI) */
