...
public final class TrackerApplication {
  ...
  private static void launchWindow() {
    ...
    // -- Status bar (con sequencer para mostrar duracion)
    final JPanel status = buildStatusBar(trackers, frame, sequencer);
    ...
    // -- Sincronizacion de compases entre canales y manejadores de eliminacion
    setupBarSync(trackers, frame, sequencer);
  }
  ...
  private static JPanel buildToolbar(final TrackerPanel[] trackers,
    ...
    bpmDown.addActionListener(e -> {
      ...
      updateDurationLabel(frame, trackers, sequencer);
    });
    ...
    // Limpiar — limpia solo el canal activo, ajusta compases al maximo del resto
    ...
    clearBtn.addActionListener(e -> {
      ...
      findByName(frame, "channelTabs", JTabbedPane.class).stream().findFirst().ifPresent(c -> {
        ...
        if (tabIdx >= 0 && tabIdx < trackers.length) {
          // Calcular maximo de compases de los OTROS canales
          int maxOthers = 1;
          for (int j = 0; j < trackers.length; j++) {
            if (j != tabIdx) maxOthers = Math.max(maxOthers, trackers[j].getNumBars());
          }
          trackers[tabIdx].clearChannel(maxOthers);
          updateDurationLabel(frame, trackers, sequencer);
        }
      });
    });
  ...
  private static JMenuBar buildMenuBar(final TrackerPanel[] trackers,
    ...
    openItem.addActionListener(e -> {
      ...
      final File loaded = MelodyFile.load(frame, trackers, sequencer,
    ...
    saveItem.addActionListener(e -> {
      if (currentFile != null) {
        MelodyFile.saveToFile(currentFile, trackers, sequencer, frame);
        frame.setTitle(DEFAULT_TITLE + " \u2014 " + currentFile.getName());
      } else {
        final File saved = MelodyFile.saveAs(frame, trackers, sequencer);
        if (saved != null) {
          currentFile = saved;
    ...
    exportItem.addActionListener(e ->
        WavExporter.export(frame, trackers, sequencer));
  ...
  private static JTabbedPane buildChannelTabs(final TrackerPanel[] trackers,
    ...
    // Pre-crear los checkboxes para poder sincronizarlos entre pestanas
    final javax.swing.JCheckBox[] checks = new javax.swing.JCheckBox[trackers.length];
    for (int i = 0; i < trackers.length; i++) {
      checks[i] = new javax.swing.JCheckBox("Vista comprimida");
      checks[i].setSelected(false);
      checks[i].setForeground(C_TEXT);
      checks[i].setBackground(C_SCALE_BG);
      checks[i].setFont(new Font("SansSerif", Font.PLAIN, 11));
      checks[i].setToolTipText("Oculta las filas fantasma y muestra solo las notas de la escala actual");
    }
    // Listener cruzado: cambiar uno actualiza todos
    for (int i = 0; i < trackers.length; i++) {
      final int fi = i;
      checks[fi].addActionListener(e -> {
        final boolean sel = checks[fi].isSelected();
        for (int j = 0; j < trackers.length; j++) {
          if (j != fi) checks[j].setSelected(sel);
          trackers[j].setCompressedView(sel);
        }
      });
    }
    ...
    for (int i = 0; i < trackers.length; i++) {
      ...
      // Franja superior: instrumento + escala + vista comprimida
      page.add(buildChannelStrip(trackers[idx], idx, checks[idx]), BorderLayout.NORTH);
  ...
  private static JPanel buildChannelStrip(final TrackerPanel tracker, final int idx,
                                           final javax.swing.JCheckBox compressChk) {
    ...
    // -- Vista comprimida (recibido como parametro, sincronizado entre pestanas)
    bar.add(compressChk);
  ...
  /** Devuelve una cadena con la duracion aproximada de la cancion segun BPM y compases. */
  private static String formatDuration(final TrackerPanel[] trackers,
                                        final SequencerEngine sequencer) {
    int maxBars = 1;
    for (final TrackerPanel tp : trackers) maxBars = Math.max(maxBars, tp.getNumBars());
    final double totalSec = maxBars * TrackerPanel.COLS * 60.0 / sequencer.getBpm() / 4.0;
    final int mins = (int)(totalSec / 60);
    final int secs = (int)(totalSec % 60);
    final int ms   = (int)Math.round((totalSec - Math.floor(totalSec)) * 1000);
    return String.format("Dur: %d:%02d.%03d  (%d compás%s, %d BPM)",
        mins, secs, ms, maxBars, maxBars > 1 ? "es" : "", sequencer.getBpm());
  }

  /** Actualiza el label de duracion en la barra de estado. */
  static void updateDurationLabel(final JFrame frame,
                                   final TrackerPanel[] trackers,
                                   final SequencerEngine sequencer) {
    findByName(frame, "durationLabel", JLabel.class)
        .forEach(l -> ((JLabel) l).setText(formatDuration(trackers, sequencer)));
  }

  /**
   * Configura la sincronizacion de compases entre canales y los manejadores
   * de eliminacion de compas. Llamar despues de que todos los componentes esten visibles.
   */
  private static void setupBarSync(final TrackerPanel[] trackers,
                                    final JFrame frame,
                                    final SequencerEngine sequencer) {
    for (int i = 0; i < trackers.length; i++) {
      final int fi = i;

      // Cuando este canal anade un compas, sincronizar el resto
      trackers[fi].setOnBarCountChanged(() -> {
        final int newCount = trackers[fi].getNumBars();
        for (int j = 0; j < trackers.length; j++) {
          if (j != fi) trackers[j].syncBarCount(newCount);
        }
        updateDurationLabel(frame, trackers, sequencer);
      });

      // Cuando el usuario solicita eliminar un compas (clic derecho)
      trackers[fi].setOnDeleteBarRequested(barIdx -> {
        // Comprobar si algún canal tiene datos en ese compas
        boolean hasData = false;
        for (final TrackerPanel tp : trackers) {
          if (tp.hasDataInBar(barIdx)) { hasData = true; break; }
        }
        if (hasData) {
          final int result = JOptionPane.showConfirmDialog(frame,
              "El compás " + (barIdx + 1) + " contiene notas en algún canal.\n"
              + "¿Deseas eliminarlo igualmente?",
              "Confirmar eliminación", JOptionPane.YES_NO_OPTION,
              JOptionPane.WARNING_MESSAGE);
          if (result != JOptionPane.YES_OPTION) return;
        }
        // Eliminar de todos los canales
        for (final TrackerPanel tp : trackers) tp.removeBarAt(barIdx);
        updateDurationLabel(frame, trackers, sequencer);
      });
    }
  }

  private static JPanel buildStatusBar(final TrackerPanel[] trackers,
                                        final JFrame frame,
                                        final SequencerEngine sequencer) {
    ...
    sep1.setForeground(C_TEXT);
    bar.add(sep1);

    // Etiqueta de duracion aproximada
    final JLabel durLbl = new JLabel(formatDuration(trackers, sequencer));
    durLbl.setForeground(C_TEXT);
    durLbl.setFont(new Font("Monospaced", Font.PLAIN, 11));
    durLbl.setName("durationLabel");
    bar.add(durLbl);

    final JLabel sep2 = new JLabel(" | ");
    sep2.setForeground(C_TEXT);
    bar.add(sep2);
