...
import es.noa.rad.fx.TremoloEffect;
...
public final class SoundEngine {
  ...
  private final TremoloEffect    tremoloEffect;

  // -------------------------------------------------------------------------
  // Arpeggiador (opcional)
  // -------------------------------------------------------------------------
  private volatile Arpeggiator arpeggiator = null;
  ...
  public SoundEngine() {
    ...
    this.tremoloEffect    = new TremoloEffect(false, 5.0, 0.6);

    this.effectChain = new EffectChain();
    this.effectChain.add(this.distortionEffect);
    this.effectChain.add(this.tremoloEffect);
   ...
   * Si el arpeggiador esta activo, la nota se encola en el arpeggiador.
  ...
  public void startNote(final Note note) {
    if (this.arpeggiator != null && this.arpeggiator.isEnabled()) {
      this.arpeggiator.noteOn(note);
    } else {
   ...
   * Si el arpeggiador esta activo, elimina la nota de su cola.
   *
  ...
  /** @return Efecto tremolo (LFO) */
  public TremoloEffect getTremoloEffect()       { return this.tremoloEffect; }

  /**
   * Vincula un arpeggiador a este motor de audio.
   *
   * @param arp Arpeggiador a usar (puede ser null para desvincularlo)
   */
  public void setArpeggiator(final Arpeggiator arp) {
    this.arpeggiator = arp;
  }

  /**
   * Dispara una voz directamente, sin pasar por el arpeggiador.
   * Usado internamente por el arpeggiador para sonar sus notas.
   *
   * @param note Nota a disparar
   */
  public void startNoteArp(final Note note) {
    this.triggerVoice(note);
  }

  /**
   * Suelta la voz de una nota directamente, sin pasar por el arpeggiador.
   *
   * @param note Nota a soltar
   */
  public void stopNoteArp(final Note note) {
    this.releaseVoiceForNote(note);
  }
  ...
  /** Dispara una voz del pool para la nota dada (logica interna, sin pasar por arpeggiador). */
  private void triggerVoice(final Note note) {
  ...
  /** Inicia el release de la voz que reproduce la nota dada (logica interna). */
  private void releaseVoiceForNote(final Note note) {
