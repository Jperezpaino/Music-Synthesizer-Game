...
public final class EffectsPanel extends JPanel {
  ...
  private static final Color COLOR_TREMOLO   = new Color(60,  190, 120);
  ...
  private JPanel buildTremoloSection(final TremoloEffect fx) {
    final JPanel panel = this.makeSectionPanel("〰  TREMOLO", COLOR_TREMOLO, fx.isEnabled(), fx::setEnabled);

    final JSlider sldRate  = makeSlider(1, 100, (int) (fx.getRate()  * 10));
    final JSlider sldDepth = makeSlider(0, 100, (int) (fx.getDepth() * 100));

    sldRate.addChangeListener(e  -> fx.setRate(sldRate.getValue()   / 10.0));
    sldDepth.addChangeListener(e -> fx.setDepth(sldDepth.getValue() / 100.0));

    addRow(panel, "Velocidad", sldRate,  "×10");
    addRow(panel, "Intensidad", sldDepth, "%");
    return panel;
  }
