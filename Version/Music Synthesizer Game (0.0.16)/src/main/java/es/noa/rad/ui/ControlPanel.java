...
import es.noa.rad.synth.InstrumentPreset;
...
public final class ControlPanel extends JPanel {
  ...
  private void buildUI() {
    ...
    // --- Sección: Instrumento (preset) ---
    gbc.gridx = 0;
    this.add(this.buildSection("INSTRUMENTO", this.buildPresetCombo()), gbc);

    // --- Sección: Forma de onda (fine-tune manual) ---
    gbc.gridx = 1;
    this.add(this.buildSection("ONDA", this.buildWaveformCombo()), gbc);
  ...
  /**
   * Crea el selector de preset de instrumento.
   */
  private JComboBox<InstrumentPreset> buildPresetCombo() {
    final JComboBox<InstrumentPreset> combo = new JComboBox<>(InstrumentPreset.values());
    combo.setBackground(COLOR_BUTTON);
    combo.setForeground(COLOR_TEXT);
    combo.setFont(new Font("SansSerif", Font.PLAIN, 13));
    combo.setPreferredSize(new Dimension(180, 30));
    combo.setFocusable(false);
    combo.setSelectedItem(this.soundEngine.getPreset());

    combo.addActionListener(e -> {
      final InstrumentPreset preset = (InstrumentPreset) combo.getSelectedItem();
      if (preset != null) {
  ...
  /**
   * Crea el selector de forma de onda (ajuste manual sobre el preset).
   */
