...
public final class TrackerPanel extends JPanel {
  ...
  /** Crea el TrackerPanel con la escala Easy + por defecto. */
  public TrackerPanel() {
    this.noteList = buildAllNotes();
    this.setBackground(C_BG);
    this.setLayout(new BorderLayout(0, 0));
    this.setBorder(BorderFactory.createEmptyBorder(8, 8, 8, 8));
    // Minimo: etiquetas + 8 columnas de CELL_W/2, al menos 12px por fila
    this.setMinimumSize(new Dimension(LABEL_W + COLS * (CELL_W / 2), ROWS * 12 + 24));
  ...
  private void buildStaticGrid(final JPanel area) {
    // GridLayout(ROWS, 1) para sincronizar altura de etiquetas con la cuadricula
    final JPanel labelsPanel = new JPanel(new GridLayout(ROWS, 1, 0, 0));
  ...
  private final class NoteLabel extends JLabel {
    ...
    NoteLabel(final int row) {
      ...
      // Solo preferred para que BorderLayout fije el ancho de la columna.
      // Sin setMaximumSize/setMinimumSize para que GridLayout pueda escalar la altura.
